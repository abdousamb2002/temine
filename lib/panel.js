...
function testSSHConnection(conn) {
  ...
  try {
    ...
    if (conn.authType === 'password') return { ok: false, error: '密码认证在 BatchMode 下无法测试，请配置 SSH Key' };
    ...
    <div class="ssh-left">
      ...
      <div id="connForm" style="display:none">
        <div class="ssh-form">
          ...
          <label>认证方式</label>
          <select id="cfAuth" onchange="document.getElementById('cfKeyRow').style.display=this.value==='key'?'':'none'">
            <option value="key">SSH Key</option>
            <option value="password">密码（终端手动输入）</option>
          </select>
          <div id="cfKeyRow"><label>Key 路径</label><input id="cfKeyPath" placeholder="~/.ssh/id_rsa" value="~/.ssh/id_rsa"></div>
...
function showConnForm(editId){
  ...
  if(!editId){document.getElementById('cfName').value='';document.getElementById('cfHost').value='';document.getElementById('cfPort').value='22';document.getElementById('cfUser').value='';document.getElementById('cfAuth').value='key';document.getElementById('cfKeyPath').value='~/.ssh/id_rsa';document.getElementById('cfKeyRow').style.display='';document.getElementById('cfEditId').value=''}
}
...
function editConn(id){
  ...
  document.getElementById('cfAuth').value=c.authType||'key';
  document.getElementById('cfKeyPath').value=c.keyPath||'~/.ssh/id_rsa';
  document.getElementById('cfKeyRow').style.display=(c.authType==='key')?'':'none';
  document.getElementById('cfEditId').value=id;
}
function saveConn(){
  var d={name:document.getElementById('cfName').value.trim(),host:document.getElementById('cfHost').value.trim(),port:parseInt(document.getElementById('cfPort').value)||22,username:document.getElementById('cfUser').value.trim(),authType:document.getElementById('cfAuth').value,keyPath:document.getElementById('cfKeyPath').value.trim()};
    ...
    if (url.pathname === '/api/ssh/connections' && req.method === 'POST') {
      readBody(req, (body) => {
        try {
          ...
          if (d.authType === 'key' && d.keyPath && /[;&|`$(){}"'\\<>\n\r]/.test(d.keyPath)) { res.writeHead(400); res.end('invalid keyPath'); return; }
          const conns = loadSSHConnections();
          const id = 'conn_' + Date.now();
          conns[id] = { id, name: d.name, host: d.host, port, username: d.username, authType: d.authType || 'key', keyPath: d.keyPath || '', lastTestResult: null };
    ...
    if (url.pathname === '/api/ssh/connections' && req.method === 'PUT') {
      readBody(req, (body) => {
        try {
          ...
          Object.assign(conns[d.id], { name: d.name || conns[d.id].name, host: d.host || conns[d.id].host, port: parseInt(d.port) || conns[d.id].port, username: d.username || conns[d.id].username, authType: d.authType || conns[d.id].authType, keyPath: d.keyPath !== undefined ? d.keyPath : conns[d.id].keyPath });
          saveSSHConnections(conns);
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ ok: true }));
        } catch { res.writeHead(400); res.end('bad request'); }
