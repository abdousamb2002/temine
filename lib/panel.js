...
export async function startPanel(port) {
  ...
  server.listen(port, '127.0.0.1', () => {
    ...
    if (process.platform === 'darwin') {
      // 优先 Chrome/Edge app mode（无地址栏独立窗口），fallback 到默认浏览器
      let opened = false;
      for (const [name, cmd] of [
        ['Google Chrome', `open -a "Google Chrome" --args --app="${url}"`],
        ['Microsoft Edge', `open -a "Microsoft Edge" --args --app="${url}"`],
      ]) {
        try { execSync(cmd); console.log(`   已在 ${name} App Mode 中打开`); opened = true; break; } catch {}
      }
