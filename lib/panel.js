...
.board-node{position:absolute;width:110px;background:#161b22;border:1px solid #30363d;border-radius:8px;cursor:grab;transition:box-shadow .15s;user-select:none;z-index:1}
...
.board-node .bn-title{font-size:11px;color:#c9d1d9;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;flex:1}
.board-node .bn-dot{width:7px;height:7px;border-radius:50%;flex-shrink:0}
...
.board-node .bn-color-btn{position:absolute;top:2px;right:2px;background:none;border:none;font-size:8px;cursor:pointer;color:#484f58;padding:2px;border-radius:3px;opacity:0;transition:opacity .15s}
.board-node:hover .bn-color-btn{opacity:1}
.board-color-picker{position:absolute;background:#161b22;border:1px solid #30363d;border-radius:8px;padding:8px;display:flex;gap:4px;flex-wrap:wrap;width:100px;z-index:20;box-shadow:0 4px 12px rgba(0,0,0,.5)}
.board-color-picker .bc-swatch{width:18px;height:18px;border-radius:4px;cursor:pointer;border:2px solid transparent;transition:all .1s}
.board-color-picker .bc-swatch:hover{border-color:#fff;transform:scale(1.2)}
...
<div id="boardView" style="display:none">
  <div class="board-toolbar">
    ...
    <span style="font-size:11px;color:#484f58">拖拽方块可自由分组，右键方块可更换颜色</span>
  </div>
  <div id="boardCanvas" class="board-container"></div>
</div>
</div>
...
function connectSSE(){
  ...
  es.onmessage=function(e){try{data=JSON.parse(e.data);if(document.getElementById('tabPanel').style.display!=='none'){if(currentView==='cards')render(data);else updateBoardFromSSE(data)}}catch(err){}};
...
function renderBoard(){
  ...
  data.windows.forEach(function(w,idx){
    ...
    html+='<div class="board-node" data-winid="'+escA(w.id)+'" style="left:'+x+'px;top:'+y+'px;border-left:4px solid '+color+'" onmousedown="startNodeDrag(event,\\''+escA(w.id)+'\\')" ondblclick="focusWin(\\''+escA(w.id)+'\\')" oncontextmenu="showColorPicker(event,\\''+escA(w.id)+'\\')"><button class="bn-color-btn" onclick="event.stopPropagation();showColorPicker(event,\\''+escA(w.id)+'\\')" title="更换颜色">🎨</button><div class="bn-header"><span class="bn-idx">'+(NUM[si-1]||si)+'</span><span class="bn-title" title="'+title+'">'+esc(w.labelText||w.name||'窗口'+si)+'</span><span class="bn-dot dot '+dotCls+'"></span></div><div class="bn-preview">'+preview+'</div></div>';
...
function updateBoardFromSSE(d){
  ...
  d.windows.forEach(function(w){
    ...
    if(node){
      ...
      if(dot)dot.className='bn-dot dot '+(w.state||'idle');
      var title=node.querySelector('.bn-title');
      if(title){var t=w.labelText||w.name||'';if(t)title.textContent=t}
    }
  });
  // 检查新窗口或已关闭的窗口
...
function showColorPicker(e,winId){
  ...
  setTimeout(function(){
    document.addEventListener('click',function closePicker(){
      picker.remove();document.removeEventListener('click',closePicker);
    });
  },0);
}
