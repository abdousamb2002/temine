...
function loadGlobalRecording() {
  try { return JSON.parse(readFileSync(GLOBAL_RECORDING_FILE, 'utf-8')); } catch { return { enabled: true }; }
}
...
async function watchLoop() {
  ...
  const tick = () => {
    ...
    tickRunning = true;
    try {
      // 全局录制关闭时跳过内容读取和日志写入，只写空状态文件
      const globalRec = loadGlobalRecording();
      if (globalRec.enabled === false) {
